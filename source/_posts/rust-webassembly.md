...
If you have some Rust code, you can compile it into [WebAssembly](https://developer.mozilla.org/en-US/docs/WebAssembly) (wasm). This tutorial takes you through all you need to know to compile a Rust project to wasm and use it in an existing web app.
...
As you might suspect, this is [the `alert` function provided by JavaScript](https://developer.mozilla.org/en-US/docs/Web/API/Window/alert). We call this function in the next section.
...
Serve the root directory of the project with a local web server, (e.g. `python3 -m http.server`). If you're not sure how to do that, refer to [Running a simple local HTTP server](https://developer.mozilla.org/en-US/docs/Learn/Common_questions/set_up_a_local_testing_server#running_a_simple_local_http_server).

Load `index.html` from the web server (if you used the python3 example: [http://localhost:8000](http://localhost:8000/)). An alert box appears on the screen, with `Hello, WebAssembly!` in it. We've successfully called from JavaScript into Rust, and from Rust into JavaScript.
...
This starts a web server. Load [http://localhost:8080](http://localhost:8080/) and an alert box appears on the screen, with `Hello, WebAssembly with NPM!` in it. We've successfully used the Rust module with npm.
